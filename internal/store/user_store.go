...
func (s *PostgresUserStore) CreateUser(user *User) error {
	query := `
  INSERT INTO users (username,email,password_hash,bio) values ($1,$2,$3,$4) RETURNING id, created_at, updated_at
  `
...
func (s *PostgresUserStore) UpdateUser(user *User) error {
	...
	result, nil := s.db.Exec(query, user.Username, user.Email, user.Bio, user.ID)
